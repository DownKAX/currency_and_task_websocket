...
class TestTasks:
    ...
    async def test_create_correct(self, client, user_session_token): #используем name, которое возвращает значение функции create_user_token
        response = await client.post("/tasks/create_task", json={"title": "todoTitl1e5", "description": "todoDescription"}, headers={"Cookie": f'session_token={user_session_token}'})
        assert 'title' in response.json()
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_create_unique_error(self, user_session_token, client):
        response = await client.post("/tasks/create_task", json={"title": "Task 1", "description": "todoDescription"}, headers={"Cookie": f'session_token={user_session_token}'})
    ...
    async def test_correct_delete_task(self, moderator_session_token, client):
        response = await client.delete("/tasks/delete_task", params={'id': 1}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 200
        assert 'title' in response.json()

    @pytest.mark.asyncio
    async def test_non_existing_delete_task(self, moderator_session_token, client):
        response = await client.delete("/tasks/delete_task", params={'id': 5}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 401
    ...
    async def test_update_task_correct_title_edit(self, moderator_session_token, client):
        response = await client.put("/tasks/update_task", params={'id': 1}, json={'title': 'updatedToDo'}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 200
    ...
    async def test_update_task_correct_description_edit(self, moderator_session_token, client):
        response = await client.put("/tasks/update_task", params={'id': 1}, json={'description': 'updatedDesc'}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 200
    ...
    async def test_update_task_correct_description_and_tilte_edit(self, moderator_session_token, client):
        response = await client.put("/tasks/update_task", params={'id': 1}, json={'title': 'updatedToDo', 'description': 'updatedDesc'},
                              headers={"Cookie": f'session_token={moderator_session_token}'})
        print(response.json())
        assert response.status_code == 200
        assert response.json()['description'] == 'updatedDesc'
        assert response.json()['title'] == 'updatedToDo'
    ...
    async def test_update_task_no_data(self, moderator_session_token, client):
        response = await client.put("/tasks/update_task", params={'id': 1}, headers={"Cookie": f'session_token={moderator_session_token}'})
    ...
    async def test_update_non_existing_task(self, moderator_session_token, client):
        response = await client.put("/tasks/update_task", params={'id': 6}, json={'title': 'updatedToDo'}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 401
        assert response.json() == {"detail": "Such Task does not exist"}

    @pytest.mark.asyncio
    async def test_finish_task_correct(self, moderator_session_token, client):
        response = await client.put("/tasks/finish_task", params={'id': 1}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 200
    ...
    async def test_finish_task_non_existing(self, moderator_session_token, client):
        response = await client.put("/tasks/finish_task", params={'id': 6}, headers={"Cookie": f'session_token={moderator_session_token}'})
        assert response.status_code == 401
        assert response.json() == {"detail": "Such Task does not exist"}
